...
with st.sidebar:
    ...
    st.session_state.ai_model = st.text_input("Tên AI Model", value=st.session_state.ai_model)
...
def background_worker(kw, api_keys, mode, ai_model="claude-sonnet-4.6", serp_manual="", linkup_manual="", rules_path="seo_geo_rules.md"):
    ...
    try:
        ...
        eng_query = ""
        for chunk in call_ai_stream(eng_p, api_keys['AI'], ai_model): eng_query += chunk
...
{{
  ...
  "faq": [{{"q": "...", "a": "..."}}]
        ...
        outline_res = ""
        for chunk in call_ai_stream(outline_p, api_keys['AI'], ai_model): outline_res += chunk
        try:
        ...
        for name, p in segments:
            ...
            update_task_status(kw, status_data)
            seg_out = ""
            for chunk in call_ai_stream(p, api_keys['AI'], ai_model): seg_out += chunk
...
Rules: RAW HTML ONLY. Use <h4> for points if needed. STRICT: NO <a> tags. Cite sources as PLAIN TEXT (e.g. According to NCBI)."""
            ...
            for chunk in call_ai_stream(h_p, api_keys['AI'], ai_model): h_out += chunk
            status_data["content"] += f"\n\n{clean_ai_html(h_out)}"
            update_task_status(kw, status_data)
...
Rules: Use <h3> for questions and <p> for answers. NO <a> tags."""
        ...
        for chunk in call_ai_stream(faq_p, api_keys['AI'], ai_model): faq_out += chunk
        ...
        for chunk in call_ai_stream(ref_p, api_keys['AI'], ai_model): ref_out += chunk
            ...
            for kw in keywords:
                ...
                if not os.path.exists(task_path):
                    ...
                    t = threading.Thread(
                        ...
                        args=(kw, st.session_state.api_keys, mode, st.session_state.ai_model, c_man, r_man)
                    )
                    t.start()
